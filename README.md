...
3. Обработка отдельного слова: из списка неизвестных слов выбирается одно слово. Подключаются парсеры значений, транскрипции, API Яндекса и еще пара-тройка написанных мной функций. В итоге, карточка слова выглядит так:

![изображение](https://user-images.githubusercontent.com/71543252/185357058-b2db4d59-77aa-4c9a-ac36-81804c627319.png)

3.1. Далее появляется меню, и перед нами предстает свобода выбора:

![изображение](https://user-images.githubusercontent.com/71543252/185353855-41ea881f-5fb9-41da-bc08-79ea729499e0.png)

3.2. Если мы выбираем "добавить", происходит следующее: слово удаляется из списка неизвестных слов и добавляется в список изучаемых слов. Далее, из вышеупомянутой карточки слова формируются четыре разные строки. Эти четыре строки для каждого слова и импортируются в Anki.

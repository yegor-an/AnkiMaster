...
У моего скрипта обширный список фич, но постараюсь вкратце разъяснить основную функциональность.

1. Обработка текста: английский текст разбивается на предложения, а затем, на слова.
2. Получение списка незнакомых слов: полученные из текста слова сравниваются с базами слов, затем, формируется список слов, у которых не было совпадений. Под базами подразумеваются списки слов, которые я знаю и которые уже находятся на изучении. Причем, учитываются и словоформы. То есть, если мне известно слово play, то скрипт "понимает", что слова plays, played и playing также мне известны и не учитывает их в формировании списка неизвестных слов. Примечание: 1 и 2 шаги выполняются только один раз для нового текста и повторяются только тогда, когда текущий список неизвестных слов полностью обработан.
3. Обработка отдельного слова: из списка неизвестных слов выбирается одно слово. Подключаются парсеры значений, транскрипции, API Яндекса и еще пара-тройка написанных мной функций.
3.1. В итоге, карточка слова выглядит так:

![изображение](https://user-images.githubusercontent.com/71543252/185357058-b2db4d59-77aa-4c9a-ac36-81804c627319.png)

3.2. Далее появляется меню, и перед нами предстает свобода выбора:

![изображение](https://user-images.githubusercontent.com/71543252/185353855-41ea881f-5fb9-41da-bc08-79ea729499e0.png)

3.3. Если мы выбираем "добавить", происходит следующее: слово удаляется из списка неизвестных слов и добавляется в список изучаемых слов. Далее, из вышеупомянутой карточки слова формируются четыре разные строки. Эти четыре строки для каждого слова и импортируются в Anki.
